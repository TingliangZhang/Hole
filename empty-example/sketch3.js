...
// 文字出现位置的横向不确定度
const BIRTH_PLACE_UNCERTAINTY_X = 20;
// 文字出现位置的旋转角不确定度
...
function drawStream(k) {
	...
	const offsetX = STREAM_OFFSET_X[k];
	const offsetY = STREAM_OFFSET_Y;
	var dx = 40*sin(radians(t2*2+k*30));
	var dxOff = 20*sin(radians(t2*2+k*20));

	if (!buffer.length || buffer[0][1].position.y - offsetY > TEXT_SIZE) {
		const box = Bodies.rectangle(offsetX + (Math.random() - 0.5) * 
				BIRTH_PLACE_UNCERTAINTY_X, offsetY, TEXT_SIZE, TEXT_SIZE);
	...
	for (let i = 0; i < buffer.length; i++) {
		...
		text(buffer[i][0], -TEXT_SIZE / 2 + dx * 0.5, TEXT_SIZE / 2);
		pop();
	}
}
